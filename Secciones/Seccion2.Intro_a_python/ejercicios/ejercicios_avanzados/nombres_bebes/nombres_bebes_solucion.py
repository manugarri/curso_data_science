...
def cargar_archivo(year):
    ...
    for linea_nombres in lineas:
        nombres = linea_nombres.split(",")
        nombres_masculinos.append(nombres[0])
        nombres_femeninos.append(nombres[2])
    return nombres_masculinos + nombres_femeninos


def calcular_popularidad_por_nombre(nombre):
    popularidad_nombre_dict = defaultdict(lambda: 0)

    for year in ["02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12", "13", "14", "15", "16"]:
        nombres_year = cargar_archivo(year)
        for nombre in nombres_year:
            popularidad_nombre_dict[nombre] += 1
    return popularidad_nombre_dict
...
def main(argumentos):
    ...
    popularidad_nombre_dict = calcular_popularidad_por_nombre(nombre)
    print("El nombre {} está entre los más populares en {} años".format(nombre, popularidad_nombre_dict[nombre]))
